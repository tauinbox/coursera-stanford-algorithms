...
class QuickSort
  ...
  attr_reader :comparisons, :result, :pivot_strategy

  def initialize(filedata, pivot_logic = 'pivot_is_median')
    @pivot_strategy = pivot_logic
    a = load_data(filedata)
    r = a.length - 1
    @comparisons = 0

    @result = quick_sort(a, 0, r, pivot_logic)
  end
...
# Usage: QuickSort.new(input_file, [strategy])
# default strategy: 'pivot_is_median'
# other strategies: 'pivot_is_first', 'pivot_is_last', 'pivot_is_random'

sorted_array = QuickSort.new(input_file)

puts sorted_array.result

puts "\n-----------------------------------------------------------"
puts "Used the '#{sorted_array.pivot_strategy}' strategy"
puts "Found #{sorted_array.comparisons} comparisons during processing"
puts "-----------------------------------------------------------\n"
